...
puts "nombre de journalist: #{journalist.count}."
a = 0
journalist.each {|x| 
	(x.length).times {|y| 
		z = x[y] 
...
puts "#{c} contient de Majuscule au début."
d = 0
journalist.each {|x|
	x.upcase
	if x.match("[:upper:]") != nil
	d += 1
	end
}
puts "#{d} contient un Majuscule."
e = 0
journalist.each {|x|
	x.capitalize
	if x.match("[_]") != nil
